...
def plot_state_probabilities(timestamps: pd.Series, posteriors: np.ndarray, path: str) -> None:
    ...
    for k in range(posteriors.shape[1]):
        plt.plot(timestamps, posteriors[:, k], label="state_%d" % k, alpha=0.8)
...
def plot_duration_hist(run_lengths, path: str) -> None:
    plt.figure(figsize=(8, 4))
    for state, lengths in run_lengths.items():
        if not lengths:
            continue
        plt.hist(lengths, bins=30, alpha=0.6, label="state_%d" % state)
...
def plot_state_posterior_heatmap(posteriors: np.ndarray, path: str) -> None:
    plt.figure(figsize=(10, 4))
    plt.imshow(posteriors.T, aspect="auto", cmap="viridis")
