...
except Exception:  # pragma: no cover - fallback for missing hmmlearn
    GMMHMM = None
    GaussianHMM = None
...
class HMMRunResult:
    ...
    entropy_p10_train: float
    entropy_p50_train: float
    entropy_p90_train: float
...
def run_hmm_grid(
            ...
            for n_mix in n_mix_list:
                ...
                for run_seed in seeds:
                    try:
                        ...
                        entropy_train = _state_entropy(post_train)
                        entropy_test = _state_entropy(post_test)
                        ...
                        result = HMMRunResult(
                            ...
                            aic=float(aic),
                            bic=float(bic),
                            entropy_mean_train=float(np.mean(entropy_train)),
                            entropy_p10_train=float(np.quantile(entropy_train, 0.1)),
                            entropy_p50_train=float(np.quantile(entropy_train, 0.5)),
                            entropy_p90_train=float(np.quantile(entropy_train, 0.9)),
                            entropy_mean_test=float(np.mean(entropy_test)),
                ...
                agg_rows.append(
                    {
                        ...
                        "entropy_train_mean": float(np.mean([r.entropy_mean_train for r in per_run])),
                        "entropy_test_mean": float(np.mean([r.entropy_mean_test for r in per_run])),
                ...
                if best_run and best_pack:
                    key = f"k{best_run.k}_cov{best_run.cov_type}_mix{best_run.n_mix}"
                    best_artifacts[key] = {
                        "run": best_run,
    ...
    return run_results, agg_rows, best_artifacts
...
def hmm_duration_diagnostics(transmat: np.ndarray, viterbi_states: np.ndarray) -> Dict[str, object]:
    ...
    runs = _run_lengths(viterbi_states)
    stats = {
