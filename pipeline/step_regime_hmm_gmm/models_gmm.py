...
def _safe_cluster_metrics(X: np.ndarray, labels: np.ndarray, max_samples: int, rng: np.random.Generator):
...
def _responsibility_entropy(resp: np.ndarray) -> np.ndarray:
    ...
    entropy = -np.sum(resp_safe * np.log(resp_safe), axis=1)
    return entropy
            ...
            agg_rows.append(
                {
                    ...
                    "silhouette_train_mean": float(
                        np.nanmean(
                            [r.silhouette_train if r.silhouette_train is not None else np.nan for r in per_run]
                        )
                    ),
                    "silhouette_test_mean": float(
                        np.nanmean(
                            [r.silhouette_test if r.silhouette_test is not None else np.nan for r in per_run]
                        )
                    ),
                    "db_train_mean": float(
