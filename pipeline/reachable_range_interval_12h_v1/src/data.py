...
def add_reachable_targets(df: pd.DataFrame, horizon: int) -> pd.DataFrame:
    ...
    y_high = high_shift.rolling(horizon, min_periods=horizon).max().shift(-(horizon - 1))
    y_low = low_shift.rolling(horizon, min_periods=horizon).min().shift(-(horizon - 1))
    df["y_high_reach"] = y_high
    df["y_low_reach"] = y_low
    df["y_range_reach"] = y_high - y_low
    df = df.dropna(subset=["y_high_reach", "y_low_reach"])
    return df.reset_index(drop=True)
...
def load_and_prepare(path: str, horizon: int) -> Tuple[pd.DataFrame, str, str]:
    ...
    df = add_reachable_targets(df, horizon)
