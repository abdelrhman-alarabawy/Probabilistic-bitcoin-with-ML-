...
def plot_yearly_summary(df: pd.DataFrame, path: str) -> None:
    ...
    for ax, target in zip(axes, targets):
        ...
        ax.plot(subset["year_month"], subset["coverage"], marker="o", label="Coverage")
        ax.plot(subset["year_month"], subset["width_pct_mean"], marker="o", label="Width % mean")
        ax.set_ylabel("Metric")
        ax.set_title(f"Coverage and Width by Month ({target})")
