...
def compute_gate_diagnostics(
    ...
    if mask.any():
        ...
        stats = {
            ...
            "p25": float(np.nanpercentile(p_trade, 25)),
            "p50": float(np.nanpercentile(p_trade, 50)),
            "p75": float(np.nanpercentile(p_trade, 75)),
            "max": float(np.nanmax(p_trade)),
        }
    else:
        ap = float("nan")
        brier = float("nan")
        stats = {"min": float("nan"), "p25": float("nan"), "p50": float("nan"), "p75": float("nan"), "max": float("nan")}
