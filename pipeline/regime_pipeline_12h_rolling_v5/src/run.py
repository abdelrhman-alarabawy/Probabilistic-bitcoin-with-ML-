...
class RobustPreprocessor(BaseEstimator, TransformerMixin):
    def __init__(self, use_pca: bool, pca_components: float):
        self.use_pca = use_pca
        self.pca_components = pca_components
        self.pipeline_ = None

    def fit(self, X: np.ndarray, y=None) -> "RobustPreprocessor":
        steps = [
...
def build_preprocessor(use_pca: bool, pca_components: float) -> RobustPreprocessor:
    return RobustPreprocessor(use_pca=use_pca, pca_components=pca_components)
...
def run() -> None:
    ...
    report_lines = []
    report_lines.append("# Regime-First Rolling Pipeline Report (12h, v5)")
    report_lines.append("")
    ...
    report_lines.append("")
    report_lines.append("## Window Setup")
    report_lines.append(
        "- Default window: 18m train / 6m test / 3m step to increase trades per test window."
    )
    report_lines.append("- Optional comparison: 18m train / 3m test / 3m step.")
    report_lines.append("")
    ...
    report_lines.append("## Window Quality Timeline")
    report_lines.append("- See figures: `figures/window_quality_*.png` (GOOD/BAD/INSUFFICIENT).")
    report_lines.append("")
