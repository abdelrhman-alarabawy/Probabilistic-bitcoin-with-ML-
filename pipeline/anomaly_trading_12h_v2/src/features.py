...
def build_event_features(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str], List[str]]:
    ...
    if INCLUDE_LIQ:
        ...
        for col in liq_cols:
            ...
            if ADD_MISSING_FLAGS:
                features[f"{col}_missing"] = series.shift(1).isna().astype(int)
    ...
    return features, base_features, liq_cols
