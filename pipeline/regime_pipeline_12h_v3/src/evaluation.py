...
def label_distribution_plot(stats: pd.DataFrame, title: str, path: str) -> None:
    if stats.empty:
        return
    x = np.arange(len(stats))
    width = 0.25
    plt.figure(figsize=(10, 4))
    plt.bar(x - width, stats["p_long"], width, label="long")
    plt.bar(x, stats["p_short"], width, label="short")
    plt.bar(x + width, stats["p_skip"], width, label="skip")
    plt.xticks(x, stats["regime_id"].astype(int))
    plt.title(title)
    plt.xlabel("Regime")
...
def frontier_plot(df: pd.DataFrame, title: str, path: str) -> None:
    ...
    plt.scatter(df["coverage"], df["precision"], alpha=0.6, label="Grid")
    pareto = df[df["pareto"]]
    if len(pareto) > 0:
        plt.scatter(pareto["coverage"], pareto["precision"], color="red", label="Pareto")
    plt.title(title)
    plt.xlabel("Coverage")
