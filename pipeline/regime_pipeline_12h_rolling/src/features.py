...
def build_features(df: pd.DataFrame, feature_shift: int = 1) -> pd.DataFrame:
    ...
    features["atr_pct"] = atr_14 / (close + 1e-12)
    ...
    features["body_to_range"] = body / ((high - low) + 1e-12)
    features["range_to_close"] = (high - low) / (close + 1e-12)

    rsi = _rsi(close, 14)
    features["rsi_14"] = rsi
    features["rsi_band"] = (rsi - 50) / 50
    features["rsi_overbought"] = (rsi - 70) / 30
    features["rsi_oversold"] = (30 - rsi) / 30
    ...
    features["bb_bandwidth"] = bb_width / (bb_mean + 1e-12)
    features["bb_pctb"] = (close - bb_lower) / (bb_width + 1e-12)
    ...
    features["obv_slope_10"] = obv.diff(10)
